...
import { BG_PRESETS, ENV_PRESETS, CAMERA_PRESETS } from '../../config/ui';
...
export function Sidebar({ state, setState, onRecordToggle, isRecording }: SidebarProps) {
  ...
  return (
    <aside className="absolute left-4 top-16 bottom-4 w-[17.5rem] glass-panel rounded-2xl flex flex-col z-20 overflow-hidden">
      ...
      <div className="flex items-center gap-0.5 px-3 pt-3 pb-2 border-b border-white/5 flex-shrink-0">
        {(['device', 'scene', 'export'] as Tab[]).map(t => (
        ...
        {tab === 'device' && (
          <div className="space-y-5 fade-up">
            ...
            <Section icon={<Upload size={13} />} title="Screen Content">
              ...
              <input ref={fileInputRef} type="file" accept="image/*,video/mp4,video/webm" onChange={handleFileChange} className="hidden" />
            </Section>
